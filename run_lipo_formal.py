...
from coarse_gnn.execution_revision import ExecutionRevision
...
def train(legacy, args, old_protocol, baseline_root, cache, factory):
    ...
    revision = ExecutionRevision(ROOT, args.output_dir, files)
    def protocol(*values):
        ...
        return revision.protocol(result, values[-1])
    def manifest(values):
        ...
        return revision.manifest(result)
    with overrides(legacy, parse_args=lambda: args, create_property_model=factory,
                   ...
                   run_single=revision.run_wrapper(legacy.run_single),
