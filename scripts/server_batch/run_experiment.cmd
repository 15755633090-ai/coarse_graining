...
"%CONDA_EXE%" run --no-capture-output -n "%CONDA_ENV%" python -u -m scripts.server_batch.run_job --bundle "%PACKAGE_ROOT%" --output-dir "%OUTPUT_ROOT%\preflight" --variant region_size_weighted --seed 0 --action prepare
if errorlevel 1 exit /b 1

echo Starting six-job GPU queue...
"%CONDA_EXE%" run --no-capture-output -n "%CONDA_ENV%" python -u -m scripts.server_batch.launch --bundle "%PACKAGE_ROOT%" --output-dir "%OUTPUT_ROOT%\jobs" --gpus 0 1
if errorlevel 1 exit /b 1
